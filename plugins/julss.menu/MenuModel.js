...
// Same classification/glyphs as the Dropbox panel (panels/dropbox/Model.js),
// so a suggested file reads consistently with the icon it would get there.
function fileGlyph(name) {
  var ext = fileExtension(name)
  if (FILE_IMAGE_EXTENSIONS[ext]) return "󰋩"
  if (FILE_VIDEO_EXTENSIONS[ext]) return "󰈫"
  if (FILE_DOCUMENT_EXTENSIONS[ext]) return "󰈙"
  return "󰈔"
}
