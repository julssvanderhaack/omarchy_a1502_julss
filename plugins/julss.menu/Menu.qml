...
Item {
  ...
  // Live "type to search files" support: while the root menu's search box is
  // non-empty, a debounced fd(1) pass over $HOME is merged in below the
  // regular results. Scoped to the root menu only -- a submenu search stays
  // about that submenu's own items, same as app rows already do.
  property var fileResults: []
  property string fileResultsQuery: ""
  property string pendingFileQuery: ""
  property string queuedFileQuery: ""
  readonly property string fileSearchScript: localPath(Qt.resolvedUrl("scripts/file-search"))

  // Own apps listing: the host does not wire root.shell.appLibrary up for
  // cloned "menu" plugins (an Omarchy platform limitation), so when it is
  // unavailable this plugin discovers .desktop entries itself instead of
  // silently showing an empty Apps menu.
  readonly property string appsListScript: localPath(Qt.resolvedUrl("scripts/list-apps"))
  ...
  // Debounces on every keystroke; a query under 2 characters is too broad to
  // be worth a filesystem walk and just clears whatever was showing.
  function scheduleFileSearch(text) {
    var query = String(text || "").trim()
    root.pendingFileQuery = query
    if (query.length < 2) {
      fileSearchDebounce.stop()
      root.fileResults = []
      root.fileResultsQuery = query.toLowerCase()
      if (root.opened && root.activeMenu === "root") root.rebuildDisplay()
      return
    }
  ...
  function startFileSearch(query) {
    if (fileSearchProc.running) {
      root.queuedFileQuery = query
      return
    }
    fileSearchProc.query = query
    fileSearchProc.collected = ""
    fileSearchProc.command = [root.fileSearchScript, query]
    fileSearchProc.running = true
  }

  function updateFileResults(raw, query) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      rows.push({ path: path, name: name, dir: dir })
    }
    rows.sort(function(a, b) {
      var aPrefix = a.name.toLowerCase().indexOf(needle) === 0 ? 0 : 1
      var bPrefix = b.name.toLowerCase().indexOf(needle) === 0 ? 0 : 1
  ...
  function fileRowsForQuery(query) {
    if (root.fileResultsQuery !== query) return []
    var home = Quickshell.env("HOME") || ""
    var rows = []
    for (var i = 0; i < root.fileResults.length; i++) {
      var entry = root.fileResults[i]
      var detail = entry.dir
      if (home && detail.indexOf(home) === 0) detail = "~" + detail.substring(home.length)
      rows.push({
        itemId: "file:" + entry.path,
        kind: "action",
        icon: MenuModel.fileGlyph(entry.name),
        iconFont: "",
        appIcon: "",
        appId: "",
        label: entry.name,
        ...
        path: "",
        childCount: 0,
        action: "gio open " + Util.shellQuote(entry.path),
        provider: "",
        score: 0,
        section: ""
      })
  ...
  // Fallback used when root.appLibrary is unavailable (see appsListScript).
  // Shells out to list-apps once per session and feeds the same
  ...
  function updateOwnAppRows(raw) {
    ...
    for (var i = 0; i < parsed.length; i++) {
      ...
      var aliases = entry.genericName ? [entry.genericName] : []
      if (Array.isArray(entry.keywords)) aliases = aliases.concat(entry.keywords)
      appRows.push({
        id: "apps." + entry.id,
        parent: "apps",
        kind: "app",
        ...
        title: "",
        target: "",
        description: String(entry.genericName || ""),
        action: "",
        provider: "",
        aliases: aliases,
        when: "",
  ...
  function rebuildDisplay() {
    ...
    if (query) {
      ...
      if (active === "root") rows = rows.concat(root.fileRowsForQuery(query.trim().toLowerCase()))
    } else {
  ...
  function setFilter(nextFilter) {
    ...
    root.scheduleFileSearch(nextFilter)
  ...
  function openDmenu(payload) {
    ...
    dmenuPrompt = String(payload.prompt || (mode === "input" ? "Entrada" : "Seleccionar"))
  ...
  Process {
    id: appsListProc
    property string collected: ""
    command: [root.appsListScript]
    stdout: SplitParser {
      onRead: function(data) { appsListProc.collected += data + "\n" }
    }
    onExited: root.updateOwnAppRows(appsListProc.collected)
  }
  ...
  Timer {
    ...
    interval: 160
    repeat: false
    onTriggered: root.startFileSearch(root.pendingFileQuery)
  }
  ...
  Process {
    ...
    onExited: {
      root.updateFileResults(fileSearchProc.collected, fileSearchProc.query)
      if (root.queuedFileQuery !== "" && root.queuedFileQuery !== fileSearchProc.query) {
        var next = root.queuedFileQuery
        root.queuedFileQuery = ""
        root.startFileSearch(next)
      } else {
        root.queuedFileQuery = ""
      }
    }
    ...
    BorderSurface {
      ...
      Item {
        ...
        ConfirmDialog {
          ...
          z: 10
          message: "¿Seguro que quieres desinstalar " + ((root.deleteTarget && root.deleteTarget.label) || "") + "?"
      ...
      Column {
        ...
        Rectangle {
          ...
          Text {
            ...
            text: root.filterText || (root.dmenuActive ? (root.dmenuPrompt + "…") : ((root.item(root.activeMenu) ? (root.item(root.activeMenu).title || root.item(root.activeMenu).label) : "Menú") + "…"))
        ...
        Item {
          ...
          ListView {
            ...
            delegate: BorderSurface {
              ...
              Image {
                ...
                source: row.isApp
                  ? (root.appLibrary ? root.appLibrary.iconSource(row.appIcon) : (row.appIcon ? Quickshell.iconPath(row.appIcon, true) : ""))
                  : ""
                asynchronous: true
                anchors.left: parent.left
          ...
          Column {
            ...
            Text {
              textFormat: Text.PlainText
              text: root.filterText ? "Sin resultados para “" + root.filterText + "”" : "Aquí no hay nada todavía"
              color: root.foreground
              opacity: 0.7
              font.family: root.fontFamily
